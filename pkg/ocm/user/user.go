...
// RemoteIDFromString
// 1. decode the base64 encoded opaque id from string
// 2. splitting the opaque id at the last @ to get the opaque id and the domain
func RemoteIDFromOpaqueID(opaqueID string) *userpb.UserId {
	remoteId := &userpb.UserId{
		Type:     userpb.UserType_USER_TYPE_FEDERATED,
		Idp:      "",
		OpaqueId: opaqueID,
	...
	bytes, err := base64.StdEncoding.DecodeString(opaqueID)
	if err != nil {
		return remoteId
	}
	remote := string(bytes)

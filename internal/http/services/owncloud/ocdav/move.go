...
func (s *svc) handleMove(ctx context.Context, w http.ResponseWriter, r *http.Request, src, dst *provider.Reference, log zerolog.Logger) {
	...
	// Check if trying to move to same location (only for path-based moves)
	if src.Path != "." && dst.Path != "." && src.ResourceId.StorageId == dst.ResourceId.StorageId && src.Path == dst.Path {
		log.Warn().Msg("attempting to move file to same location")
		w.WriteHeader(http.StatusForbidden)
		b, err := errors.Marshal(http.StatusForbidden, "cannot move file to same location", "", "")
		errors.HandleWebdavError(&log, w, b, err)
		return
	}
